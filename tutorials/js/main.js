...
(function () {
  ...
  function getByLongName (longname) {
    var i; var l = reflection.length; var x
    for (x = 0; x < l; x += 1) {
  ...
  function drawTocClass (parentList, i) {
    ...
    ['Params', 'Properties', 'Methods', 'Events'].forEach(function (memberType) {
      var n = memberType.toLowerCase(); var t
      ...
      i[n].sort(alpha).forEach(function (member) {
        ...
        var memberLink = ce('a', 'toc-link', memberLi)
  ...
  function drawToc (parentNode) {
    var ca
    var ct
    var toc = ce('div', 'toc', parentNode)
    var tocList = ce('ul', 'toc-list', toc)
    var tutorialsUl
    ca = ce('a', null, toc)
    ca.id = 'toc_tutorials'
  ...
  function drawMemberTable (members, memberType, parentNode, parentMember) {
    ...
    var trh = ce('tr', 'member-header-row', table)
    var headers = {
  ...
  function drawTopic (parentNode, longname) {
    ...
    ['Params', 'Properties', 'Methods', 'Events', 'Classes'].forEach(function (memberType) {
      var n = memberType.toLowerCase(); var tt
      if (!i[n] || i[n].length === 0) { return }
      if (top) {
    ...
    if (i.name && top) {
      ...
      Object.keys(tutorials).forEach(function (tutorialKey) {
        ...
        var tutorialForm = ce('form', ['child-topic', 'tutorial'], parentNode)
        var tan = ce('a', ['tutorial-anchor', 'topic-anchor'], tutorialForm)
        var theading = ce('h2', ['tutorial-heading', 'topic-heading'], tutorialForm)
        var tdescription = ce('p', 'tutorial-description', tutorialForm)
        var aceEditor
        var fiddleButton = ce('button', ['jsfiddlelogo', 'tutorial-button'], tutorialForm)
        var executeButton = ce('button', 'tutorial-button', tutorialForm)
        var error = ce('div', 'tutorial-error', tutorialForm)
        var code = ce('div', 'tutorial-code', tutorialForm)
        var gridParent = ce('div', 'tutorial-grid', tutorialForm)
        var hiddenFormItems = {}
        var fiddleForm = {
  ...
  function drawHeader (parentNode) {
    ...
    var tocLink = ce('a', 'top-link', parentNode)
    tocLink.href = '#toc'
    tocLink.innerHTML = '\u2191'
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addEventListener('hashchange', function () {
      var hashTocTarget; var hashTarget; var hash = getHash()
      if (hash) {
