...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function plotSparklineChart (cell, ctx) {
    ...
    var gb
    var x = 0
    var d = (cell.value[0] - cell.value[1]).toFixed(2)
    var m = Math.max.apply(null, cell.value)
    var a = cell.value.reduce(function (ac, c) { return ac + c }, 0) / cell.value.length
    var i = Math.min.apply(null, cell.value)
    var w = cell.width / cell.value.length
    var ar = (d > 0 ? '\u25BC' : '\u25B2')
    var r = cell.height / (m - (m * 0.1))
    function line (n, c) {
      ctx.beginPath()
      ctx.lineWidth = 1
      ctx.strokeStyle = c
  ...
  function getData (fill) {
    var a; var x = grid.scrollIndexRect.top
