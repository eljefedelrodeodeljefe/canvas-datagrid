...
/* globals Event: true, canvasDatagrid: false, alert: false */
document.addEventListener('DOMContentLoaded', function () {
  ...
  var loadStyle = document.createElement('button')
  var copyCode = document.createElement('button')
  var newButton = document.createElement('button')
  var saveButton = document.createElement('button')
  var deleteButton = document.createElement('button')
  var styleLibSelect = document.createElement('select')
  var help = document.createElement('button')
  var titleCanvas = document.createElement('canvas')
  var ctx = titleCanvas.getContext('2d')
  var code
  var table = document.createElement('table')
  var sLength = Object.keys(grid.defaults.styles).length
  var colorInputs = {}
  var fontSize = 60
  var titleCanvasHeight = 75
  var titleCanvasWidth = 430
  var storageKey = 'canvas-datagrid-user-style-library'
  var tdls = {}
  var inputs = {}
  ...
  function drawTitleCanvas () {
    ...
    var l
    var ty
    var fExp = 'px ' + inputs.activeCellFont.value.replace(/\d+px/, '')
    var keys = Object.keys(grid.defaults.styles).filter(function (key) { return /Color|Style/.test(key) })
    var borders = keys.filter(function (key) { return (/border/i).test(key) })
    var notBorders = keys.filter(function (key) { return !/border/i.test(key) })
  ...
  function pickColor (getColor, onchange) {
    return function () {
      var v = getColor(); var i = document.createElement('input')
  ...
  function fillStyle (el) {
    ...
    var er; var userLib = window.localStorage.getItem(storageKey)
    try {
  ...
  function selectStyleInput (e) {
    ...
    var a = e.cell.active ? 'active' : ''
    var aKey = a + s + 'BackgroundColor'
    var cKey = a + s + 'Color'
    var oKey = a + s + 'BorderColor'
  ...
  function setupDemoGrid () {
    function createData (n) {
      var x; var data = []; var d; var i; var c
  ...
  saveButton.onclick = function () {
    ...
    var storedValues
    try {
      storedValues = JSON.parse(window.localStorage.getItem(storageKey)) || {}
    } catch (e) {
  ...
  deleteButton.onclick = function () {
    ...
    window.localStorage.setItem(storageKey, JSON.stringify(window.styleLibrary))
    fillStyle(styleLibSelect)
    drawTitleCanvas()
