...
import { maybeInjectPassive } from './helpers'

export default function intf (self, ctor) {
  self.scale = 1
  self.orders = {
    rows: [],
    columns: []
  ...
  self.getSelectedData = function (expandToRow) {
    var d = []; var s = self.getSchema(); var l = self.data.length
    if (l === 0) { return [] }
  ...
  self.setStorageData = function () {
    ...
    window.localStorage.setItem(self.storageName + '-' + self.attributes.name, JSON.stringify({
      sizes: {
  ...
  self.filter = function (type) {
    ...
    if (!f && type !== undefined) {
      console.warn('Cannot find filter for type %s, falling back to substring match.', type) // eslint-disable-line no-console
  ...
  self.dispose = function () {
    ...
    self.eventParent.removeEventListener('wheel', self.scrollWheel, maybeInjectPassive())
  ...
  self.reloadStoredValues = function () {
    if (self.attributes.name && self.attributes.saveAppearance) {
      try {
        self.storedSettings = window.localStorage.getItem(self.storageName + '-' + self.attributes.name)
      } catch (e) {
        console.warn('Error loading stored values. ' + e.message) // eslint-disable-line no-console
      ...
      if (self.storedSettings) {
        try {
          ...
          console.warn('could not read settings from localStore', e) // eslint-disable-line no-console
          self.storedSettings = undefined
        }
      }
      if (self.storedSettings) {
  ...
  self.init = function () {
    ...
    self.filters.string = function (value, filterFor) {
      ...
      var filterRegExp
      var regEnd = /\/(i|g|m)*$/
      var pattern = regEnd.exec(filterFor)
      var flags = pattern ? pattern[0].substring(1) : ''
