...
export default function dom (self) {
  self.getClippingRect = function (ele) {
    var boundingRect = self.position(self.parentNode)
    var eleRect = self.position(ele)
    var s = self.scrollOffset(self.canvas)
    var clipRect = {
      x: 0,
      y: 0,
      h: 0,
  ...
  self.scrollOffset = function (e) {
    var x = 0; var y = 0; var scrollingElement = document.scrollingElement || { scrollLeft: 0, scrollTop: 0 }
  ...
  self.resizeEditInput = function () {
    if (self.input && self.input.editCell) {
      ...
      var s = self.scrollOffset(self.intf)
      var bm = self.style.gridBorderCollapse === 'collapse' ? 1 : 2
      var borderWidth = (self.style.cellBorderWidth * bm)
      var cell = self.getVisibleCellByIndex(self.input.editCell.columnIndex, self.input.editCell.rowIndex) ||
  ...
  self.position = function (e, ignoreScrollOffset) {
    var x = 0; var y = 0; var s = e; var h; var w
  ...
  self.setDom = function () {
    ...
    self[self.isChildGrid ? 'parentGrid' : 'eventParent'].addEventListener('wheel', self.scrollWheel, maybeInjectPassive())
