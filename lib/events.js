...
/* globals define: true, requestAnimationFrame: false */
define([], function () {
  'use strict'
  return function (self) {
    ...
    self.resize = function (drawAfterResize) {
      ...
      var b = (self.style.scrollBarBorderWidth * 2)
      var d = self.style.scrollBarBoxMargin * 0.5
      var sbw = self.style.scrollBarWidth + (self.style.scrollBarBorderWidth * 2)
      var ratio = self.getRatio()
      var bm = self.style.gridBorderCollapse === 'collapse' ? 1 : 2
      var cellBorder = self.style.cellBorderWidth * bm
      var columnHeaderCellBorder = self.style.columnHeaderCellBorderWidth * bm
      var dataHeight = 0
      var dataWidth = 0
      var dims
    ...
    self.mousemove = function (e, overridePos) {
      ...
      var x = self.mouse.x
      var y = self.mouse.y
      var o = self.getCellAt(x, y)
      var delta
      var ev = { NativeEvent: e, cell: o, x: x, y: y }
      var previousCell = self.currentCell
    ...
    self.click = function (e, overridePos) {
      var i
      var startingBounds = JSON.stringify(self.getSelectionBounds())
      var ctrl = (e.ctrlKey || e.metaKey || self.attributes.persistantSelectionMode)
      var pos = overridePos || self.getLayerPos(e)
      self.currentCell = self.getCellAt(pos.x, pos.y)
      if (self.currentCell.grid !== undefined) {
        return
      }
      function checkSelectionChange () {
        var ev; var sb = self.getSelectionBounds()
    ...
    self.dragReorder = function (e) {
      var pos; var x; var y
      var columReorder = self.dragMode === 'column-reorder'
      var rowReorder = self.dragMode === 'row-reorder'
      pos = self.getLayerPos(e)
    ...
    self.mousedown = function (e, overridePos) {
      ...
      var move = /-move/.test(self.dragMode)
      var freeze = /frozen-row-marker|frozen-column-marker/.test(self.dragMode)
      var resize = /-resize/.test(self.dragMode)
    ...
    self.keydown = function (e) {
      var i
      var ev
      var adjacentCells = self.getAdjacentCells()
      var x = self.activeCell.columnIndex
      var y = self.activeCell.rowIndex
      var ctrl = (e.ctrlKey || e.metaKey)
      var last = self.data.length - 1
      var s = self.getSchema()
      var cols = s.length - 1
    ...
    self.keypress = function (e) {
      ...
      if (self.dispatchEvent('keypress', { NativeEvent: e, cell: self.currentCell })) { }
    }
    ...
    self.scrollWheel = function (e) {
      ...
      var t
      var ev = e
      var deltaX = e.deltaX === undefined ? e.NativeEvent.deltaX : e.deltaX
      var deltaY = e.deltaY === undefined ? e.NativeEvent.deltaY : e.deltaY
      var deltaMode = e.deltaMode === undefined ? e.NativeEvent.deltaMode : e.deltaMode
      if (wheeling) {
        return
      }
    ...
    self.pasteItem = function (clipData, x, y, mimeType) {
      var l; var s = self.getVisibleSchema(); var yi = y - 1; var sel = []
    ...
    self.getNextVisibleColumnIndex = function (visibleColumnIndex) {
      var x; var s = self.getVisibleSchema()
      for (x = 0; x < s.length; x += 1) {
    ...
    self.copy = function (e) {
      ...
      var textRows = []
      var outputHeaders = {}
      var outputHeaderKeys
      var sData = self.getSelectedData()
      var s = self.getSchema()
      function htmlSafe (v) {
