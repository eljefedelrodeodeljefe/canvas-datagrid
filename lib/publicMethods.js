...
export default function publicMethods (self) {
  /**
  ...
  self.gotoCell = function (x, y, offsetX, offsetY) {
    ...
    var targetY = y === undefined ? undefined : self.findRowScrollTop(y)
    var cell
    var sbw = self.scrollBox.width - (self.scrollBox.verticalBarVisible ? self.style.scrollBarWidth : 0)
    var sbh = self.scrollBox.height - (self.scrollBox.horizontalBarVisible ? self.style.scrollBarWidth : 0)
  ...
  self.forEachSelectedCell = function (fn, expandToRow) {
    var d = []; var s = expandToRow ? self.getSchema() : self.getVisibleSchema(); var l = self.data.length
  ...
  self.selectRow = function (rowIndex, ctrl, shift, supressEvent) {
    var x; var st; var en; var s = self.getVisibleSchema()
    function de () {
      if (supressEvent) { return }
      self.dispatchEvent('selectionchanged', {
        selectedData: self.getSelectedData(),
  ...
  self.expandTree = function (rowIndex) {
    ...
    var rowHeaderCellWidth = self.sizes.columns.cornerCell || self.style.rowHeaderCellWidth
    var h = self.sizes.trees[rowIndex] || self.style.treeGridHeight
    var treeGrid
  ...
  self.isCellVisible = function (cell, rowIndex) {
    ...
    var x; var l = self.visibleCells.length
    for (x = 0; x < l; x += 1) {
  ...
  self.order = function (columnName, direction, sortFunction, dontSetStorageData) {
    var f
    var c = self.getSchema().filter(function (col) {
    ...
    if (!f && c[0].type !== undefined) {
      console.warn('Cannot sort type "%s" falling back to string sort.', c[0].type) // eslint-disable-line no-console
    }
  ...
  self.getCellAt = function (x, y, useTouchScrollZones) {
    ...
    var border
    var tsz = useTouchScrollZones ? self.attributes.touchScrollZone : 0
    var moveMode = self.attributes.borderDragBehavior === 'move'
    var i
    var l = self.visibleCells.length
    var moveBorder
    var xBorderBehavior = moveMode ? self.cursorGrab : 'ew-resize'
    var yBorderBehavior = moveMode ? self.cursorGrab : 'ns-resize'
    var cell
    var entitiy
  ...
  self.getSelectionBounds = function () {
    ...
    var high = { x: -Infinity, y: -Infinity }
    self.selections.forEach(function (row, rowIndex) {
      var maxCol, minCol
  ...
  self.selectArea = function (bounds, ctrl) {
    ...
    var ev; var x; var y; var s = self.getSchema()
    if (!ctrl) {
      self.selections = []
    }
