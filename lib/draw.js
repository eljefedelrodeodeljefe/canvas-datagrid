...
/* globals Image: true, requestAnimationFrame: false, performance: false, btoa: false */

export default function draw (self) {
  var perfCounters = []
  var cachedImagesDrawn = false
  var drawCount = 0
  var perfWindowSize = 300
  var entityCount = []
  // TODO(eljefedelrodeo): review after lint fix
  // let hiddenFrozenColumnCount = 0
  var scrollDebugCounters = []
  var touchPPSCounters = []
  self.htmlImageCache = {}
  ...
  function drawHtml (cell) {
    var img
    var v = cell.innerHTML || cell.formattedValue
    var cacheKey = v.toString() + cell.rowIndex.toString() + cell.columnIndex.toString()
    var x = cell.x + self.canvasOffsetLeft
    var y = cell.y + self.canvasOffsetTop
  ...
  function drawOrderByArrow (x, y) {
    ...
    var ml = self.style.columnHeaderOrderByArrowMarginLeft * self.scale
    var mr = self.style.columnHeaderOrderByArrowMarginRight * self.scale
    var aw = self.style.columnHeaderOrderByArrowWidth * self.scale
    var ah = self.style.columnHeaderOrderByArrowHeight * self.scale
    x += self.canvasOffsetLeft
    y += self.canvasOffsetTop
  ...
  function drawTreeArrow (cell, x, y) {
    ...
    var mr = self.style.treeArrowMarginRight * self.scale
    var ml = self.style.treeArrowMarginLeft * self.scale
    var aw = self.style.treeArrowWidth * self.scale
    var ah = self.style.treeArrowHeight * self.scale
    x += self.canvasOffsetLeft
    y += self.canvasOffsetTop
  ...
  function wrapText (cell, splitChar) {
    ...
    var x
    var word
    var words = cell.formattedValue.split(splitChar)
    var textHeight = cell.calculatedLineHeight
    var lines = []
    // var out = []
    var wrap = self.style.cellWhiteSpace !== 'nowrap'
    var elWidth
    var et = self.attributes.ellipsisText
    // var elClipLength
    var plWidth
    var clippedVal
    // var ogWordWidth
    ...
    var cHeight = wrap ? cell.paddedHeight : cell.calculatedLineHeight
    lines.push(line)
  ...
  function drawText (cell) {
    ...
    var h = (cell.fontHeight * cell.lineHeight)
    var x
    var line
    var wrap = self.style.cellWhiteSpace !== 'nowrap'
    var textHeight = 0
    for (x = 0; x < cell.text.lines.length; x += 1) {
      ...
      var hPos = cell.paddingLeft + cell.treeArrowWidth + cell.orderByArrowWidth
  ...
  function getFrozenColumnsWidth () {
    ...
    var x = 0
    var n = Math.min(self.frozenColumn, s.length)
    var column
  ...
  self.draw = function (internal) {
    ...
    // initial values
    var checkScrollHeight; var rowHeaderCell; var p; var cx; var cy; var treeGrid; var rowOpen
    var rowHeight; var cornerCell; var y; var x; var c; var h; var w; var s; var r; var rd; var aCell
    var data = (self.data || [])
    var bc = self.style.gridBorderCollapse === 'collapse'
    var selectionBorders = []
    var moveBorders = []
    var selectionHandles = []
    var rowHeaders = []
    var l = data.length
    var u = self.currentCell || {}
    var columnHeaderCellHeight = self.getColumnHeaderCellHeight()
    var rowHeaderCellWidth = self.getRowHeaderCellWidth()
    var cellHeight = self.style.cellHeight
    drawCount += 1
    p = performance.now()
    ...
    function drawScrollBars () {
      var drawCorner
      var en = self.scrollBox.entities
      var m = (self.style.scrollBarBoxMargin * 2)
    ...
    function drawCell (d, rowOrderIndex, rowIndex) {
      return function drawEach (header, headerIndex, columnOrderIndex) {
        ...
        var cellGridAttributes
        var cell
        var isHeader = /HeaderCell/.test(cellStyle)
        var isCorner = /cornerCell/.test(cellStyle)
        var isRowHeader = cellStyle === 'rowHeaderCell'
        var isColumnHeader = cellStyle === 'columnHeaderCell'
        var selected = self.selections[rowOrderIndex] && self.selections[rowOrderIndex].indexOf(columnOrderIndex) !== -1
        var hovered = self.hovers.rowIndex === rowOrderIndex && self.hovers.columnIndex === columnOrderIndex
        var active = self.activeCell.rowIndex === rowOrderIndex && self.activeCell.columnIndex === columnOrderIndex
        var isColumnHeaderCellCap = cellStyle === 'columnHeaderCellCap'
        var rawValue = d ? d[header.name] : undefined
        var isGrid = header.type === 'canvas-datagrid'
        var activeHeader = (self.orders.rows[self.activeCell.rowIndex] === rowOrderIndex ||
        ...
        var val
        var f = self.formatters[header.type || 'string']
        var orderByArrowSize = 0
        var treeArrowSize = 0
        var cellWidth = self.sizes.columns[headerIndex] || header.width
        var ev = {
          value: rawValue,
          row: d,
          header: header
    ...
    function drawHeaders () {
      var d; var g = s.length; var i; var o; var columnHeaderCell; var header; var nonFrozenHeaderWidth
    ...
    function drawRow (r, d) {
      var i; var treeHeight; var rowSansTreeHeight; var o; var g = s.length
      if (y - (cellHeight * 2) > h) {
        return false
      }
      rd = data[r]
    ...
    function drawFrozenRows () {
      var n; var ln = Math.min(data.length, self.frozenRow)
      x = -self.scrollBox.scrollLeft + self.scrollPixelLeft + self.style.cellBorderWidth
    ...
    function drawRows () {
      ...
      var o; var n; var i; var g = s.length
      x = -self.scrollBox.scrollLeft + self.scrollPixelLeft + self.style.cellBorderWidth
    ...
    function drawSelectionHandles () {
      if (self.mobile || self.attributes.allowMovingSelection) {
        ...
        selectionHandles.forEach(function (c) {
          ...
          var ax = c[0].x + (c[1] === 'tl' || c[1] === 'bl' ? 0 : c[0].width) - az
          var ay = c[0].y + (c[1] === 'bl' || c[1] === 'br' ? c[0].height : 0) - az
          self.visibleCells.unshift({
            x: ax,
            y: ay,
    ...
    function drawActiveCell () {
      ...
      var acx = cl ? self.lastFrozenColumnPixel : 0
      var acy = cl ? self.lastFrozenRowPixel : 0
      var acw = cl ? self.width - self.lastFrozenColumnPixel : self.width
      var ach = cl ? self.height - self.lastFrozenRowPixel : self.height
    ...
    function drawFrozenMarkers () {
      ...
      var mx = self.lastFrozenColumnPixel - self.style.frozenMarkerBorderWidth
      var xHover = self.currentCell && self.currentCell.style === 'frozen-row-marker'
      var yHover = self.currentCell && self.currentCell.style === 'frozen-column-marker'
    ...
    function drawPerfLines () {
      ...
      var pw = 250
      var px = self.width - pw - self.style.scrollBarWidth - (self.style.scrollBarBorderWidth * 2)
      var py = columnHeaderCellHeight
      var ph = 100
